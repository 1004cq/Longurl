(() => {
  ...
  const isPureUrl = (value) => {
    const text = String(value || '').trim();
    if (!text) return false;
    const clean = extractUrl(text);
    if (!clean) return false;
    return text === clean || text === `https://${clean}` || text === `http://${clean}`;
  };
  ...
  createApp({
    setup() {
      ...
      const submit = async () => {
        ...
        if (url.value.trim() && extractUrl(url.value) !== url.value.trim() && !/^https?:\/\//i.test(url.value.trim())) {
      ...
      return { copy, lang, langUrl, isLost, min, max, url, length, loading, error, result, copied, preview, ratio, normalize, isPureUrl, setLength, updateUrl, submit, paste, onPaste, copyResult };
    },
    template: `
